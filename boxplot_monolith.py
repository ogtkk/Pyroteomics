...
df_TFA_temp['log2'] = df_TFA_temp[u'AVERAGE: ' + 'Tip'] / df_TFA_temp[u'AVERAGE: ' + 'Solution']
...
df_HFBA_temp['log2'] = df_HFBA_temp[u'AVERAGE: ' + 'Tip'] / df_HFBA_temp[u'AVERAGE: ' + 'Solution']
...
df_merge['HFBA/TFA'] = np.log2(df_merge['log2_HFBA'] / df_merge['log2_TFA'])
...
bins = map(lambda x: float(x) * 40, range(12))
df_merge['RetTime_bin'] = pd.cut(df_merge.RetTime_HFBA, bins)
fig = plt.figure()
fig.patch.set_color('white')
ax1 = fig.add_subplot(111)
...
bp = df_merge.boxplot(column='HFBA/TFA', by='RetTime_bin', grid=False, whis=[5,95],
           ax=ax1, positions=map(lambda x: float(x) + 0.5, range(11)),
           whiskerprops=whisprop, capprops=capprop, boxprops=boxprop,
           medianprops=medprop, flierprops=flprop, return_type='dict')
[[item.set_color('black') for item in bp[key]['medians']] for key in bp.keys()]
[[item.set_color('black') for item in bp[key]['boxes']] for key in bp.keys()]
...
ax1.set_xticks(range(len(bins)))
ax1.set_xticklabels([0,40,80,120,160,200,240,280,320,360,400,440,480,520,560,600])
ax1.set_xlim(2,11)
fig.suptitle('')
ax1.set_title('')
ax1.set_xlabel('Retention time (min)', labelpad=20)
ax1.set_ylabel('Relative reporter ion intensity\n$Log_2((HFBA/Solution)/(TFA/Solution))$')
ax1.set_title('n=%d' % len(df_merge))
plt.grid(False)
plt.grid(axis='y')
plt.subplots_adjust(bottom=0.2, left=0.2, right=0.8)
plt.savefig('boxplot_xRetTyInt-monHFBA-TFA160403_2.png')
plt.clf()

data_t = pd.DataFrame()
if bool(reporter_values) is True:
    for cn in reporter_values:
        series = df_temp.ix[:, df_temp.columns.map(
            lambda x: x.startswith(cn))]
        data_t = pd.concat([data_t, series], axis=1)
data_t = data_t.join(df_temp[u'SeqModModDetail'])
data_t = data_t.replace(0, np.nan)
for title, reporter in reporters.items():
    data_temp = data_temp.groupby([u'SeqModModDetail']).mean()
    data_lnInt = pd.DataFrame()
    data_Int = data_temp.iloc[:, data_temp.columns.isin(reporter)]
    for cn in reporter:
        data_lnInt[cn] = np.log10(data_Int[cn])
    fig_title = title + ' (n=%d)' % len(data_lnInt)
    def set_color(cv):
        if cv <= 20:
            return "blue"
        else:
    ...
    lnAverage = [np.nanmean(data_lnInt.loc[i]) for i in data_lnInt.index]
    data_result['avr'] = [np.nanmean(data_Int.loc[i]) for i in data_Int.index]
    data_result['stdev'] = [np.std(data_Int.loc[i]) for i in data_Int.index]
    data_result['cv'] = data_result['stdev'] / data_result['avr'] * 100
    color_list = map(set_color, data_result.cv)
    plt.scatter(data_result['cv'], lnAverage, c=color_list, s=8, alpha=0.6)
    plt.xlim(0, 100)
    plt.ylim(2, 7)
    plt.axvline(x=20, ls='--', lw=0.8, color='black')
    plt.text(20, 6.2, "C.V. $\leqq$ 20\n(n = %d)" % len(data_result[data_result['cv'] <= 20]), size=14)
